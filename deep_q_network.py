...
class DeepQNetwork(nn.Module):
    def __init__(self, lr, n_actions, name, input_dims, chkpt_dir):
        ...
        generations_embedding = nn.Embedding(POKEMON_EXTRA_SMALL_VOCAB_SIZE, embedding_size, input_length=1 )
        embeddings.append(generations_embedding)
        embeddings_shape.append(Reshape(target_shape=(embedding_size,)))

        emb_dims = [(POKEMON_EXTRA_SMALL_VOCAB_SIZE, POKEMON_EXTRA_SMALL_EMBEDDINGS_DIM)]
    ...
    def forward(self, state):
        ...
        conv2 = F.reul(self.conv2(conv1))
        conv3 = F.relu(self.conv3(conv2))
        # conv3 shape is BS x n_filters x H x W
        conv_state = conv3.view(conv3.size()[0], -1)
        flat1 = F.relu(self.fc1(cconv_state))
